...
int main() {
    ...
    rep(i, n) {
        rep(j, n) {
            ...
            bool owari = true;
            rep(k, m) {
                if ((ans[i + 1][j] == '.' || !f[ans[i + 1][j] - 'a'][c[k].se]) && (ans[i][j + 1] == '.' || !f[ans[i][j + 1] - 'a'][c[k].se])) {
                    ans[i + 1][j + 1] =  c[k].se + 'a';

                    if (ans[i + 1][j] != '.') f[ans[i + 1][j] - 'a'][c[k].se] = true;;
                    if (ans[i][j + 1] != '.') f[ans[i][j + 1] - 'a'][c[k].se] = true;;

                    rep(l, m) {
                        if (ans[i + 1][j] != '.' && ans[i + 1][j] - 'a' == c[l].se) c[l].fi++;
                        if (ans[i][j + 1] != '.' && ans[i][j + 1] - 'a' == c[l].se) c[l].fi++;
                    }
