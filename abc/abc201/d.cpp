...
int main() {
    ...
    vvl dpt(h + 2, vl(w + 2, -LINF));
    vvl dpa(h + 2, vl(w + 2, -LINF));
    dpt[1][1] = 0;
    dpa[1][1] = 0;
    ...
    repp(i, h) repp(j, w) {
        if ((i + j) % 2 == 0) {
            if (i < h) {
                umax(dpt[i + 1][j], dpt[i][j] + a[i + 1][j]);
                umax(dpa[i + 1][j], dpa[i][j]);
            }
            if (j < w) {
                umax(dpt[i][j + 1], dpt[i][j] + a[i][j + 1]);
                umax(dpa[i][j + 1], dpa[i][j]);
            }
        } else {
            if (i < h) {
                umax(dpa[i + 1][j], dpa[i][j] + a[i + 1][j]);
                umax(dpt[i + 1][j], dpt[i][j]);
            }
            if (j < w) {
                umax(dpa[i][j + 1], dpa[i][j] + a[i][j + 1]);
                umax(dpt[i][j + 1], dpt[i][j]);
            }
        }
    }
