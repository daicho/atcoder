...
ll solve(ll y, ll x, bool t) {
    if (t) {
        if (y == h && x == w) {
            return a[y][x];
        } else if (y == h) {
            return solve(y, x + 1, !t) + a[y][x];
        } else if (x == w) {
            return solve(y + 1, x, !t) + a[y][x];
        } else {
            return max(solve(y, x + 1, !t), solve(y + 1, x, !t)) + a[y][x];
        }
    } else {
        if (y == h && x == w) {
            return -a[y][x];
            ...
            return min(solve(y, x + 1, !t), solve(y + 1, x, !t)) + a[y][x];
        }
    }
}
