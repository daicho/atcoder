...
template<ll m> struct mint {
    ...
    mint& operator-=(mint a) { if ((x += m - a.x) >= m) x -= m; return *this; }
    mint& operator*=(mint a) { (x *= a.x) %= m; return *this; }
    mint& operator/=(mint a) { return *this *= a.inv(); }
    friend istream& operator>>(istream& is, mint& a) { return is >> a.x; }
    friend ostream& operator<<(ostream& os, mint& a) { return os << a.x; }
    mint inv() { return pow(m - 2); }
    mint pow(ll t) { mint a = x, r = 1; while (t) { if (t & 1) r *= a; a *= a; t >>= 1; } return r; }
};
