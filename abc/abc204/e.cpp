...
int main() {
    ...
    rep(i, m) {
        g[a[i]].pb({b[i], {c[i], d[i]}});
        g[b[i]].pb({a[i], {c[i], d[i]}});
    }
    ...
    while (!q.empty()) {
        ...
        for (auto tt: g[t.se]) {
            q.push({t.fi + tt.se.fi + (tt.se.se / (t.fi + 1)), tt.fi});
            ld ttt = max((ld)0, sqrtl(tt.se.se) - 1);
            if (ttt > t.fi) {
                ll mi = ll(ttt) + tt.se.fi + (tt.se.se / (ll(ttt) + 1));
                ll tttt;
                if (umin(mi, ll(ttt + 1) + tt.se.fi + (tt.se.se / (ll(ttt + 1) + 1)))) {
                    tttt = ll(ttt + 1);
                } else {
                    tttt = ll(ttt);
                }

                ll mt = tttt + tt.se.fi + (tt.se.se / (tttt + 1));

                while (tttt > t.fi) {
                    if (tttt - 1 + tt.se.fi + (tt.se.se / (tttt)) > mt) {
                        break;
                    }
                ...
                q.push({tttt + tt.se.fi + (tt.se.se / (tttt + 1)), tt.fi});
            }
        }
    }
