...
int main() {
    ...
    vl y(m), x(m);
    rep(i, m) cin >> y[i] >> x[i], y[i]--, x[i]--;

    set<pii> s;
    rep(i, m) s.insert(mp(y[i], x[i]));

    vector<pii> sy(h), sx(w);
    rep(i, h) sy[i] = mp(0, i);
    rep(i, w) sx[i] = mp(0, i);
    ...
    rep(i, h) {
        if (sy[i].fi != sy[0].fi) break;
        rep(j, w) {
            if (sx[j].fi != sx[0].fi) break;

            if (s.find(mp(sy[i].se, sx[j].se)) == s.end()) {
                prt(sy[0].fi + sx[0].fi);
                return 0;
            };
        }
