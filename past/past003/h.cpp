...
int main() {
    ...
    rep(i, l) {
        if (x[i + 1]) {
            umin(dp[i + 1], dp[i] + t[0] + t[2]);
        } else {
            umin(dp[i + 1], dp[i] + t[0]);
        }

        if (i + 2 <= l) {
            if (x[i + 2]) {
                umin(dp[i + 2], dp[i] + t[0] + t[1] + t[2]);
            } else {
                umin(dp[i + 2], dp[i] + t[0] + t[1]);
            }
        } else {
            umin(dp[l], dp[i] + (t[0] + t[1]) / 2);
        }

        if (i + 4 <= l) {
            if (x[i + 4]) {
                umin(dp[i + 4], dp[i] + t[0] + 3 * t[1] + t[2]);
            } else {
                umin(dp[i + 4], dp[i] + t[0] + 3 * t[1]);
            }
        } else {
            umin(dp[l], dp[i] + (t[0] + t[1]) / 2 + (l - i - 1) * t[1]);
        }
    }
